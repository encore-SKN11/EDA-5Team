...
## 프로젝트 소개
이 프로젝트는 영화 데이터에 대해 탐색적 데이터 분석을 수행합니다.
<br/>
영화의 수익`(revenue, ROI(Return On Investment))`에 있어 어떤 요소가 상관관계를 보여주는지에 중점을 두었습니다.

<br/>

## 💡 프로젝트 필요성

- 영화에 관심이 있는 사람들에게 유용한 Insight 제공
- 예를 들어, 흥행에 실패한 영화들이 가진 요소들을 반면교사 삼아 해당 단점을 어떻게 극복할 것인지 논의해볼 수 있음.
- 반면에, 그러한 요소들을 가지고도 흥행에 성공한 영화들이 어떤 데이터 외적 요소를 가지고 있는지 추가적인 분석을 진행해볼 수 있음.
...
- 어떤 감독의 인기작이 많다는 뜻은 곧 해당 감독의 역량이 높을 확률이 크다는 뜻이므로, 흥행에 영향을 끼침.
...
- 그러나 인기영화를 많이 찍은 실력파 배우라고 해도, 영화의 수익이 높음을 보장해주지는 않는 것을 보임.

- 좀 더 정밀한 분석을 위해서는 주/조연 데이터가 포함된 분석을 진행할 필요가 있어보임.
...
### 📊 시각화 및 인사이트
#### 💡 '폭망'의 기준?
- ROI (Return on Investment) = (수익 / 제작비) × 100
...
🔹 **'제작비/평점/추천수'와 수익 간의 관계** : 대체로 **제작비가 많을수록 높은 수익을 기록**하는 경향이 있음. 의외로 **평점과 수익간의 관계는 낮은 편**이며, **추천수와 수익간의 관계가 높음**.
<br/><br/>

🔹 **장르별 분석**: 액션, 모험 장르가 평균적으로 높은 수익을 기록했으며, **비평적으로는 드라마와 다큐멘터리가 높은 평점**을 받음.
<br/><br/>

🔹 **감독과 배우의 영향**: 특정 감독은 **흥행 성적에 영향을 미침.** (예: 크리스토퍼 놀란, 스티븐 스필버그 등) **그러나 인기배우의 경우에는 단순히 출연했다고만 해서 흥행 성적에 영향을 주지는 않음.**
<br/><br/>
